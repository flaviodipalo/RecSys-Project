...
class Movielens1MReader(object):
    def __init__(self, train_test_split=None, train_validation_split=None, delete_popular=None, k_cores=None, delete_interactions=None, top_popular_threshold = 0.33, delimiter = "::"):
        ...
        self.URM_test = sps.csr_matrix((self.ratings[test_mask], (self.users[test_mask], self.movies[test_mask])),shape=(max_user_id,max_item_id))

        self.URM_train = sps.csr_matrix((self.ratings[train_mask], (self.users[train_mask], self.movies[train_mask])),shape=(max_user_id,max_item_id))
