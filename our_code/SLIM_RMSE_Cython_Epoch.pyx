...
cdef class SLIM_RMSE_Cython_Epoch:
    ...
    def __init__(self, URM_train, learning_rate, gamma, beta, iterations, gradient_option,similarity_matrix_normalized, topK):
        ...
        if self.similarity_matrix_normalized:
            ...
            self.S = Sparse_Matrix_Tree_CSR(self.n_movies, self.n_movies)

        #ADAGRAD
        self.adagrad_cache = np.zeros((self.n_movies), dtype=float)

        #ADAM
        self.adam_m = np.zeros((self.n_movies), dtype=float)
        self.adam_v = np.zeros((self.n_movies), dtype=float)
        self.beta_1 = 0.9
        self.beta_2 = 0.999
        self.time_t = 0

        #RMSprop
        self.rms_prop_term = np.zeros((self.n_movies), dtype=float)
                    ...
                    if self.similarity_matrix_normalized:
                        ...
                        for index in range(URM_indices[URM_indptr[user_index]:URM_indptr[user_index+1]].shape[0]):
                            ...
                            if target_user_index != j:
                                non_zero_gradient[support_index] = partial_error*URM_data[URM_indptr[user_index]:URM_indptr[user_index+1]][index] + i_beta*self.S[target_user_index, j] + i_gamma
                                adagrad_cache[target_user_index] += non_zero_gradient[support_index]**2

                                '''
                                if gradient_option == "adagrad":
                                    adagrad_cache[target_user_index] += (non_zero_gradient[support_index])**2
                                    non_zero_gradient[support_index] = (1/sqrt(adagrad_cache[target_user_index] + eps))*non_zero_gradient[support_index]
                    ...
                    for index in range(URM_indices[URM_indptr[user_index]:URM_indptr[user_index+1]].shape[0]):
                        ...
                        if target_user_index != j:
                            if self.similarity_matrix_normalized:
                                ...
                                gradient = partial_error*URM_data[URM_indptr[user_index]:URM_indptr[user_index+1]][index] + i_beta*self.S.get_value(target_user_index, j) + i_gamma

                            if gradient_option == adagrad_option:
                                if self.similarity_matrix_normalized:
                                    self.S[target_user_index, j] -= (alpha/sqrt(sum_gradient)/n_movies + eps)*gradient
                                ...
                                self.adam_m[target_user_index] = self.beta_1*self.adam_m[target_user_index] + (1-self.beta_1)*gradient
                                self.adam_v[target_user_index] = self.beta_2*self.adam_v[target_user_index] + (1-self.beta_2)*(gradient)**2
                                self.m_adjusted = self.adam_m[target_user_index]/(1 - self.beta_1**self.time_t)
                                self.v_adjusted = self.adam_v[target_user_index]/(1 - self.beta_2**self.time_t)
                                self.S[target_user_index] -= alpha*self.m_adjusted/(sqrt(self.v_adjusted) + eps)
#
                            elif gradient_option == rmsprop_option:
                                self.rms_prop_term[target_user_index] = 0.9*self.rms_prop_term[target_user_index] + 0.1*gradient**2
                                self.S[target_user_index] -= alpha*gradient/(sqrt(self.rms_prop_term[target_user_index] + eps))
    ...
    def get_S(self):
        S = self.S.get_scipy_csr(self.topK)
        print("NNNZ", S.nnz)
        print(S[:100, :])
        return S
