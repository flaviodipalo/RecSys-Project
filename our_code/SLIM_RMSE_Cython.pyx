...
import time
import sys
from useful_functions import check_matrix
...
cdef class SLIM_ElasticNet_Cython:
    ...
    cdef double[:] some_useful_double_variable, URM_train_data
    cdef bint sparse_weights
    ...
    def __init__(self, URM_train,
                 learning_rate = 0.05,
                 topK=100, sgd_mode='adagrad',sparse_weitghts = 0):

        #super(SLIM_ElasticNet_Cython, self).__init__()

        URM_train = check_matrix(URM_train, 'csr')

        #some definitions added by us:
        self.sparse_weights = sparse_weitghts
    ...
    def epochIteration_Cython(self):
        ...
        cdef long start_time_batch = time.time()
                ## we add the definition of sparse weights as bint
