...
cdef class SLIM_RMSE_Cython_Epoch:
    ...
    cdef S_sparse
    cdef int[:] S_indptr, adagrad_indptr
    cdef int[:] S_indices, adagrad_indices
    cdef double[:] S_data, adagrad_data
    ...
    def __init__(self, URM_train, learning_rate, gamma, beta, iterations, gradient_option, similarity_matrix_normalized, topK=None):
        ...
        #ADAGRAD
        adagrad_cache = sp.sparse.random(self.n_movies, self.n_movies, format='csc', density=0.0000001)
        self.adagrad_indptr = adagrad_cache.indptr
        self.adagrad_indices = adagrad_cache.indices
        self.adagrad_data = adagrad_cache.data
