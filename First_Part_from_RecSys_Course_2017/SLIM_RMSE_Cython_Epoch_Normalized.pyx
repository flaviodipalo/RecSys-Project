...
import random
#call(shlex.split('python3 /home/alessio/PycharmProjects/RecSys_Project/First_Part_from_RecSys_Course_2017/SLIM_RMSE/setup.py build_ext --inplace'))
...
cdef double vector_sum_normalization(double[:] vector):
...
cdef double vector_sum(Sparse_Matrix_Tree_CSR vector, int j, int length):

    cdef int i
    cdef double counter = 0

    for i in range(length):
        counter += vector.get_value(i, j)
...
#@cython.boundscheck(False)
cdef class SLIM_RMSE_Cython_Epoch_Normalized:
    ...
    def epochIteration_Cython(self):
        ...
        cdef bint found
        cdef int index_for_found_flag, index_to_insert
        ...
        for j in range(0, n_movies):
            ...
            if self.similarity_matrix_normalized:
                sum_vector = vector_sum_normalization(S_data)
            ...
            for t_index in range(all_items_indices[all_items_indptr[j]:all_items_indptr[j+1]].shape[0]):
                ...
                if URM_indices[URM_indptr[user_index]:URM_indptr[user_index+1]].shape[0] > 1:
                    ...
                    if self.similarity_matrix_normalized:
                        sum_gradient = vector_sum(adagrad_cache, j, self.n_movies)
                        p_index = 0
                    for index in range(URM_indices[URM_indptr[user_index]:URM_indptr[user_index+1]].shape[0]):
                        target_user_index = URM_indices[URM_indptr[user_index]:URM_indptr[user_index+1]][index]
...
#
            ...
            for index_for_support in range(self.topK):
                ...
                index_to_insert = int(support_matrix_indices[j, index_for_support])
                value_to_insert = vals[index_to_insert]
