...
class EpinionsReader(object):
    ...
    def __init__(self, train_test_split=None, train_validation_split=None, delete_popular=None,
                 top_popular_threshold=0.33):
        '''
        :param train_test_split: is the percentage of the training set
        '''

        dir = os.path.dirname(__file__)
        filename = dir + "/ratings.csv"
        ...
        rows, cols, vals = [], [], []
        numCells = 0
        #fileHandle['item_id'], levels = pd.factorize(fileHandle['item_id'])
        #fileHandle['user_id'], levels = pd.factorize(fileHandle['user_id'])
...
EpinionsReader(train_validation_split=[0.2,0.2,0.6])
